...
import { governedAdvancePlaybook, governedAudit, governedAutomationState, governedCases, governedCreateCase, governedEvaluateAutomations, governedImprovementState, governedIngest, governedKpis, governedObservability, governedPlaybookState, governedRecordBacktest, governedRefresh, governedStartPlaybook, governedUpdateCase, hasPermission, initializePlatform, platformSession, switchGovernedRole, } from "./api.js";
...
import { addAudit, loadAudit, loadCases, loadRole, resetLocalState, saveCases, saveDatasetName, saveRole, seedCases, updateCase, } from "./storage.js";
...
let role = loadRole();
let view = roleLanding[role];
let cases = loadCases();
let audit = loadAudit();
let platformModeState = "static-showcase";
let governedKpiCatalog = [];
...
let playbooks = [];
let playbookRuns = [];
let valueRealization = { initiativesActive: 0, initiativesCompleted: 0, successful: 0, inconclusive: 0, hoursSavedMonthly: 0, backlogAvoided: 0, slaImprovementPoints: 0, measuredInitiatives: [] };
let analystQuery = "What should management prioritize?";
let analystAnswer = null;
...
function deltaMarkup(value, goodWhen, suffix = "%") {
...
function kpiCard(title, value, note, delta, sparkValues, sparkDirection, accent = "default") {
...
function button(label, action, style = "secondary", iconName) {
...
function shell(content) {
    ...
    <aside class="sidebar" aria-label="Primary navigation">
      ...
      <nav class="nav-list">${visibleNav
        .map((item) => `<button type="button" class="nav-item ${view === item.id ? "nav-item--active" : ""}" data-nav="${item.id}">${icon(item.icon)}<span>${escapeHtml(item.label)}</span>${item.id === "workflow" && cases.filter((entry) => entry.status !== "Resolved").length > 0 ? `<b>${cases.filter((entry) => entry.status !== "Resolved").length}</b>` : ""}</button>`)
        .join("")}</nav>
      <div class="sidebar__footer"><div class="dataset-chip"><span class="status-dot"></span><div><small>Active dataset</small><strong title="${escapeHtml(dataset.name)}">${escapeHtml(dataset.name)}</strong></div></div><div class="version-line">Release v${APP_VERSION}<small>${escapeHtml(APP_BUILD)}</small></div></div>
    </aside>
...
function renderOverview() {
    ...
    return `${sectionHeader("Service Operations Command Center", "Turn operational signals into accountable action", "Validated performance metrics, explainable alerts, root-cause evidence, assignments, and measured follow-up in one decision workflow.", `${hasPermission("ingest_data") ? button("Upload dataset", "open-upload", "secondary", "upload") : ""}${button("Export management brief", "export-brief", "primary", "download")}`)}
...
function renderAlertCompact(item) {
...
function filteredRecords() {
    ...
    return dataset.records.filter((record) => {
        ...
        if (filters.priority !== "All" && record.priority !== filters.priority)
            return false;
        if (record.createdAt && record.createdAt < start)
            return false;
        if (search) {
...
function filterBar(includeSearch = false) {
...
function renderAnalysis() {
    ...
    return `${sectionHeader("Analysis Lab", "Diagnose movement and compare scenarios", "Filters recalculate the visible KPIs, trends, anomalies, and root-cause rankings against the selected operational slice.", `${button("Export filtered records", "export-filtered", "secondary", "download")}${button("Create management brief", "export-brief", "primary", "download")}`)}
  ${filterBar()}
...
function renderRootCauseTable(items) {
    if (!items.length)
        return emptyState("No root-cause ranking", "More recent closed records are required.");
...
function renderAlertDetailed(item) {
...
function currentAutomationMetrics() {
    if (!dataset || !analytics)
        return {};
    return {
...
function simulateLocalAutomations() {
...
function deriveValueSnapshot(items) {
...
function renderProcessIntelligence() {
    if (!dataset || !analytics || !intelligence)
        return "";
    ...
    return `${sectionHeader("Process Intelligence", "See how work flows, where it stalls, and which patterns deserve investigation", "Derived process paths combine request attributes with actual cycle-time, SLA, and reopen evidence. Association signals are explicit and do not claim causality.", button("Open supporting records", "go-records", "secondary", "table"))}
...
function renderAutomationImprovement() {
    if (!dataset || !analytics || !intelligence)
        return "";
    const executions = automationExecutions.slice(0, 8);
    const activeRun = playbookRuns.find((item) => item.status === "Active") ?? playbookRuns[0];
    return `${sectionHeader("Automation & Improvement", "Turn governed signals into controlled action—and prove whether the action worked", "Rules use Trigger → Logic → Action with simulation, cooldown, deduplication, case assignment, playbooks, problem management, and value realization.", `${button("Simulate rules", "simulate-automations", "secondary", "analysis")}${platformModeState === "server-governed" && hasPermission("run_automation") ? button("Run governed rules", "execute-automations", "primary", "automation") : ""}`)}
...
function pulseForRole() {
    if (!intelligence)
        return [];
    if (role === "Data Steward")
        return intelligence.pulse.filter((item) => item.id === "pulse-quality" || item.id === "pulse-signal");
    if (role === "Operator")
        return intelligence.pulse.filter((item) => item.id === "pulse-backlog" || item.id === "pulse-signal");
    return intelligence.pulse;
}
function renderOperationsAnalyst() {
    if (!dataset || !analytics || !intelligence)
        return "";
    ...
    return `${sectionHeader("Operations Analyst", "Personalized signals and evidence-grounded operational Q&A", "The analyst answers only from the currently loaded governed metrics, process evidence, and documented assumptions; it does not fabricate records or hide metric definitions.")}
...
function renderWorkflow() {
    ...
    return `${sectionHeader("Action & Follow-up", "Convert evidence into owned work", "Assign analytical findings, track status and targets, and preserve a complete audit history of operational decisions.", `${button("Add top recommendation", "add-top-case", "secondary", "plus")}${button("Export case register", "export-cases", "primary", "download")}`)}
...
function renderCase(item) {
    ...
    return `<article class="case-card"><div class="case-card__rail case-card__rail--${severityClass(item.priority)}"></div><div class="case-card__body"><div class="case-card__header"><div><span class="severity-label severity-label--${severityClass(item.priority)}">${escapeHtml(item.priority)}</span><span class="status-pill status-pill--${statusClass(item.status)}">${escapeHtml(item.status)}</span></div><small>${escapeHtml(item.id)} · Due ${formatDate(item.dueAt, { month: "short", day: "numeric" })}</small></div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p><div class="case-card__grid"><label><span>Status</span><select data-case-status="${escapeHtml(item.id)}">${["Open", "In Progress", "Monitoring", "Resolved"].map((status) => `<option value="${status}" ${status === item.status ? "selected" : ""}>${status}</option>`).join("")}</select></label><label><span>Owner</span><input data-case-owner="${escapeHtml(item.id)}" value="${escapeHtml(item.owner)}" maxlength="80"></label><div><span>Expected impact</span><strong>${escapeHtml(item.expectedImpact)}</strong></div></div><div class="case-measure"><div class="case-measure__header"><span>${escapeHtml(item.baselineMetric)}</span><strong>${formatNumber(item.currentValue, 1)}${escapeHtml(item.unit)} / target ${formatNumber(item.targetValue, 1)}${escapeHtml(item.unit)}</strong></div><div class="progress-track"><span style="width:${progress}%"></span></div><small>Baseline ${formatNumber(item.baselineValue, 1)}${escapeHtml(item.unit)} · ${Math.round(progress)}% of target movement achieved</small></div></div></article>`;
}
function renderAudit(events) {
    if (!events.length)
        return emptyState("No audit events", "Workflow actions will appear here.");
...
function renderGovernance() {
    if (!dataset || !analytics)
        return "";
    const issuesBySeverity = (severity) => dataset.quality.issues.filter((item) => item.severity === severity).length;
    return `${sectionHeader("Data Governance", "Make the analytical trust layer visible", "Quality rules, KPI definitions, data lineage assumptions, exceptions, and audit evidence are part of the product—not hidden documentation.", `${button("Download quality issues", "export-quality", "secondary", "download")}${hasPermission("ingest_data") ? button("Upload replacement data", "open-upload", "primary", "upload") : ""}`)}
...
function renderObservability() {
    ...
    return `${sectionHeader("System Health", "Operate the analytics platform, not just the dashboard", "Release identity, governed ingestion, data quality, workflow follow-through, measured outcomes, and refresh health are visible as service-level objectives.", platformModeState === "server-governed" && hasPermission("ingest_data") ? button("Check governed source now", "refresh-source", "secondary", "reset") : "")}
...
async function recordCurrentBacktestEvidence() {
    if (platformModeState !== "server-governed" || !dataset || !hasPermission("record_backtests"))
        return;
    const backtest = runScenario(dataset.records, dataset.quality, 0, 0).backtest;
    await governedRecordBacktest(backtest);
}
async function syncGovernedState() {
    ...
    const automationState = await governedAutomationState().catch(() => ({ rules: [], executions: [] }));
    automationRules = automationState.rules;
    automationExecutions = automationState.executions;
    const improvementState = await governedImprovementState().catch(() => ({ problems: [], initiatives: [], value: valueRealization }));
    problems = improvementState.problems;
    initiatives = improvementState.initiatives;
    valueRealization = improvementState.value;
    const playbookState = await governedPlaybookState().catch(() => ({ playbooks: [], runs: [] }));
...
async function switchRole(nextRole) {
    if (platformModeState === "server-governed") {
        ...
        if (dataset && hasPermission("record_backtests")) {
...
function renderQualityIssues(items) {
    if (!items.length)
        return emptyState("No quality exceptions", "All visible quality rules passed.");
...
function renderRecords() {
    ...
    return `${sectionHeader("Record Explorer", "Drill from KPIs to source-level evidence", "Search, filter, inspect, and export the records supporting the visible operational conclusions.", `${hasPermission("ingest_data") ? button("Upload CSV", "open-upload", "secondary", "upload") : ""}${button("Export filtered CSV", "export-filtered", "primary", "download")}`)}
  ${filterBar(true)}
...
function renderRecordsTable(records, compact) {
...
function renderAbout() {
    ...
    return `${sectionHeader("Product Story", "A closed-loop operations platform—not just a dashboard", "The platform demonstrates how trusted operational evidence becomes an explanation, a controlled action, a measured outcome, and an improvement decision.", `${button("Download project README", "download-readme", "secondary", "download")}${button("Open command center", "go-overview", "primary", "dashboard")}`)}
  <section class="story-hero"><div><span class="eyebrow">Decision narrative</span><h2>Observe → explain → act → verify</h2><p>A reviewer can trace a business problem from source records through governed metrics and process evidence, test a controlled automation, start a playbook, and see how improvement value is measured—all with transparent assumptions and drill-down evidence.</p><div class="story-tags"><span>Data analysis</span><span>Process intelligence</span><span>Business intelligence</span><span>Data governance</span><span>Workflow automation</span><span>Operations</span><span>Program management</span><span>Analytics engineering</span></div></div><div class="story-score"><strong>12</strong><span>linked stages from data to operational learning</span></div></section>
...
function recompute(nextDataset) {
...
async function loadStaticIntelligenceCatalogs() {
    if (platformModeState !== "static-showcase")
        return;
    ...
    if (automationResponse.ok) {
        ...
        automationRules = (payload.rules ?? []).map((rule) => ({
            id: String(rule.id ?? ""), name: String(rule.name ?? ""), description: String(rule.description ?? ""), enabled: Boolean(rule.enabled), severity: String(rule.severity ?? "Medium"),
            ...
            conditions: Array.isArray(rule.conditions) ? rule.conditions : [], action: (rule.action ?? {}),
        }));
    }
    if (playbookResponse.ok) {
        const payload = await playbookResponse.json();
    ...
    if (improvementResponse.ok) {
        const payload = await improvementResponse.json();
...
async function handleUpload(file) {
    if (file.size > 15 * 1024 * 1024)
        throw new Error("The demo upload limit is 15 MB.");
...
function managementBriefHtml() {
    ...
    const issueCounts = (severity) => dataset.quality.issues.filter((item) => item.severity === severity).length;
...
async function generateAiSummary() {
    if (!dataset || !analytics || summaryMode === "pending")
        return;
    summaryMode = "pending";
    render();
    const payload = {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`AI summary endpoint returned ${response.status}.`);
        const result = await response.json();
        if (!result.summary || result.summary.length < 80)
            throw new Error("AI summary response was incomplete.");
...
async function createCaseFromRecommendation(recommendationId) {
    if (!analytics)
        return;
    const rec = analytics.recommendations.find((item) => item.id === recommendationId);
    if (!rec || cases.some((item) => item.title === rec.title))
        return;
    const now = new Date();
...
function addTopCase() {
    ...
    if (available)
        void createCaseFromRecommendation(available.id).catch((error) => showToast(error instanceof Error ? error.message : String(error), "error"));
    else
        showToast("All current recommendations already have cases.", "warning");
}
...
root.addEventListener("click", (event) => {
    ...
    if (action) {
        if (action === "toggle-sidebar") {
            ...
            exportFiltered();
        else if (action === "export-quality")
            exportQuality();
        else if (action === "export-cases")
            exportCases();
        else if (action === "export-brief")
            exportBrief();
        else if (action === "generate-ai-summary")
            void generateAiSummary();
        else if (action === "copy-summary")
            void navigator.clipboard.writeText(summaryText).then(() => showToast("Summary copied to the clipboard.")).catch(() => showToast("Clipboard access is unavailable.", "warning"));
        else if (action === "clear-filters")
            resetFilters();
        else if (action === "download-template")
            downloadTemplate();
        else if (action === "download-readme")
            downloadReadme();
            ...
            void (async () => {
                if (platformModeState !== "server-governed")
                    throw new Error("Governed automation execution requires the local server mode.");
            ...
            const field = document.querySelector("#routing-text");
            routingText = field?.value.trim() || routingText;
            routingSuggestion = classifyRequestText(routingText);
            render();
        }
        else if (action === "ask-analyst") {
            const field = document.querySelector("#analyst-query");
            analystQuery = field?.value.trim() || analystQuery;
            if (dataset && analytics && intelligence)
                analystAnswer = answerOperationsQuestion(analystQuery, analytics, intelligence, dataset.name);
            render();
        }
        else if (action === "add-top-case")
            addTopCase();
            ...
            if (platformModeState === "static-showcase")
                resetLocalState();
            void (async () => {
                if (platformModeState === "server-governed" && hasPermission("ingest_data")) {
                    await governedRefresh();
                    await syncGovernedState();
                }
                await loadDemo();
    ...
    const caseButton = target.closest("[data-create-case]");
    if (caseButton?.dataset.createCase)
        void createCaseFromRecommendation(caseButton.dataset.createCase).catch((error) => showToast(error instanceof Error ? error.message : String(error), "error"));
    const playbookButton = target.closest("[data-start-playbook]");
    if (playbookButton?.dataset.startPlaybook) {
        ...
        void (async () => {
            ...
            showToast("Playbook started.");
            render();
        })().catch((error) => showToast(error instanceof Error ? error.message : String(error), "error"));
    }
    const advanceButton = target.closest("[data-advance-playbook]");
    if (advanceButton?.dataset.advancePlaybook) {
        const runId = advanceButton.dataset.advancePlaybook;
        void (async () => {
            if (platformModeState === "server-governed") {
                await governedAdvancePlaybook(runId);
                await syncGovernedState();
            }
            else {
            ...
            showToast("Playbook progress recorded.");
            render();
        })().catch((error) => showToast(error instanceof Error ? error.message : String(error), "error"));
    }
    const analystQuestion = target.closest("[data-analyst-question]")?.dataset.analystQuestion;
    if (analystQuestion) {
...
root.addEventListener("change", (event) => {
    ...
    if (target.id === "role-select") {
        void switchRole(target.value).catch((error) => {
    ...
    if (target.dataset.filter) {
        ...
        if (key === "dateWindowDays")
            filters.dateWindowDays = Number(target.value);
        else if (key === "search")
...
root.addEventListener("input", (event) => {
    const target = event.target;
    if (target.id === "analyst-query")
        analystQuery = target.value;
    else if (target.id === "routing-text")
        routingText = target.value;
        ...
        window.__searchTimer = window.setTimeout(() => render(), 180);
    }
});
