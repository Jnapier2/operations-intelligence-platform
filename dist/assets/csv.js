...
export function toServiceRecords(rows) {
...
function issue(record, ruleId, field, severity, message, value) {
    return {
...
function scoreRule(id, name, dimension, description, severity, failed, total) {
...
export function validateRecords(records) {
    ...
    for (const record of records) {
        if (!record.requestId)
            add(issue(record, "required-id", "request_id", "Critical", "Request ID is required.", record.requestId));
        if (!record.createdAt)
            add(issue(record, "valid-created", "created_at", "Critical", "Created timestamp is missing or invalid.", record.raw.created_at));
        if (!record.status)
            add(issue(record, "required-status", "status", "High", "Status is required.", record.status));
        if (!record.category)
            add(issue(record, "required-category", "category", "High", "Category is required.", record.category));
        if (!record.location)
            add(issue(record, "required-location", "location", "Medium", "Location is required for geographic analysis.", record.location));
        if (!record.team)
            add(issue(record, "required-team", "team", "High", "Team is required for accountability reporting.", record.team));
        if (!record.owner)
            add(issue(record, "required-owner", "owner", "Medium", "Owner is required for work assignment.", record.owner));
        if (!KNOWN_CATEGORIES.has(record.category))
            add(issue(record, "known-category", "category", "High", "Category is not mapped to the controlled taxonomy.", record.category));
        if (!KNOWN_PRIORITIES.has(record.priority))
            add(issue(record, "known-priority", "priority", "High", "Priority is not recognized.", record.priority));
        if (!KNOWN_STATUSES.has(record.status))
            add(issue(record, "known-status", "status", "High", "Status is not recognized.", record.status));
        if (record.slaHours === null || record.slaHours <= 0)
            add(issue(record, "valid-sla", "sla_hours", "High", "SLA hours must be a positive number.", record.raw.sla_hours));
        ...
        if (record.status === "Closed" && !record.closedAt)
            add(issue(record, "closed-consistency", "closed_at", "High", "Closed records require a closed timestamp.", record.raw.closed_at));
        if (record.status !== "Closed" && record.closedAt)
            add(issue(record, "open-consistency", "closed_at", "Medium", "Open records should not have a closed timestamp.", record.raw.closed_at));
        if (record.resolutionHours !== null && record.resolutionHours < 0)
            add(issue(record, "valid-resolution", "resolution_hours", "Critical", "Resolution hours cannot be negative.", record.resolutionHours));
        if (!record.lastUpdatedAt)
            add(issue(record, "valid-updated", "last_updated_at", "Medium", "Last-updated timestamp is missing or invalid.", record.raw.last_updated_at));
        if (record.requestId) {
    ...
    const severityWeight = { Critical: 4.0, High: 2.5, Medium: 1.5, Low: 0.75 };
...
export function trustedRecords(records, report) {
    const blockingRows = new Set(report.issues
...
export async function loadCsvDataset(url) {
    ...
    if (missing.length > 0)
        throw new Error(`Dataset is missing required columns: ${missing.join(", ")}.`);
