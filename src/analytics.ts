...
function isClosed(record: ServiceRecord): boolean {
  return record.status === "Closed" && validDate(record.closedAt);
}

function metSla(record: ServiceRecord): boolean {
  return isClosed(record) && record.slaHours !== null && record.resolutionHours !== null && record.resolutionHours <= record.slaHours;
}

function backlogAt(records: ServiceRecord[], when: Date, predicate?: (record: ServiceRecord) => boolean): number {
  return records.filter((record) => {
    if (predicate && !predicate(record)) return false;
    if (!validDate(record.createdAt) || record.createdAt > when) return false;
    return !validDate(record.closedAt) || record.closedAt > when;
  }).length;
}

function analysisDateFor(records: ServiceRecord[]): Date {
  let latest = Number.NEGATIVE_INFINITY;
  for (const record of records) {
    for (const value of [record.lastUpdatedAt, record.closedAt, record.createdAt]) {
      if (validDate(value)) latest = Math.max(latest, value.getTime());
    }
  }
  if (!Number.isFinite(latest)) throw new Error("No trusted records with valid timestamps are available for analysis.");
  return new Date(latest);
}

function slaRate(records: ServiceRecord[]): number {
  const closed = records.filter(isClosed);
  return percentage(closed.filter(metSla).length, closed.length);
}

function reopenRate(records: ServiceRecord[]): number {
  const closed = records.filter(isClosed);
  return percentage(closed.filter((record) => record.reopened).length, closed.length);
}

function satisfactionAverage(records: ServiceRecord[]): number {
  return mean(records.map((record) => record.satisfactionScore).filter((value): value is number => value !== null && value >= 1 && value <= 5));
}

function resolutionMedian(records: ServiceRecord[]): number {
  return median(records.filter(isClosed).map((record) => record.resolutionHours).filter((value): value is number => value !== null && value >= 0));
}

export function computeKpis(records: ServiceRecord[], quality: QualityReport): KpiSet {
...
export function buildDailyTrend(records: ServiceRecord[], quality: QualityReport, days = 42): DailyPoint[] {
  const trusted = trustedRecords(records, quality);
...
function groupCount(records: ServiceRecord[], key: (record: ServiceRecord) => string): Map<string, number> {
...
function toSortedMetrics(groups: Map<string, number>, limit = 12): DimensionMetric[] {
...
export function backlogByCategory(records: ServiceRecord[], quality: QualityReport): DimensionMetric[] {
  const trusted = trustedRecords(records, quality);
  const analysisDate = analysisDateFor(trusted);
...
export function slaByTeam(records: ServiceRecord[], quality: QualityReport): DimensionMetric[] {
  const trusted = trustedRecords(records, quality);
...
export function volumeByLocation(records: ServiceRecord[], quality: QualityReport): DimensionMetric[] {
  const trusted = trustedRecords(records, quality);
  const end = analysisDateFor(trusted);
  const start = addDays(end, -29);
...
function severityFor(change: number, evidence: number, kind: "volume" | "sla" | "backlog"): Severity {
  const magnitude = Math.abs(change);
  if (kind === "sla") {
    if (magnitude >= 25 && evidence >= 10) return "Critical";
    if (magnitude >= 16 && evidence >= 8) return "High";
    if (magnitude >= 10 && evidence >= 5) return "Medium";
    return "Low";
  }
  if (kind === "volume") {
    if (magnitude >= 100 && evidence >= 25) return "Critical";
    if (magnitude >= 60 && evidence >= 15) return "High";
    if (magnitude >= 30 && evidence >= 10) return "Medium";
    return "Low";
  }
  if (magnitude >= 100 && evidence >= 20) return "Critical";
  if (magnitude >= 50 && evidence >= 12) return "High";
  if (magnitude >= 25 && evidence >= 8) return "Medium";
  return "Low";
}

function alertId(prefix: string, dimension: string, name: string): string {
  return `${prefix}-${dimension}-${name}`.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
}

export function detectAlerts(records: ServiceRecord[], quality: QualityReport): AlertItem[] {
  const trusted = trustedRecords(records, quality);
  const end = analysisDateFor(trusted);
  ...
  const dimensions: Array<{ label: "Category" | "Team" | "Location"; value: (record: ServiceRecord) => string }> = [
  ...
  for (const dimension of dimensions) {
    ...
    for (const name of names) {
      const matches = (record: ServiceRecord) => dimension.value(record) === name;
  ...
  const typeRank: Record<AlertItem["type"], number> = { "Service Level": 5, Backlog: 4, "Data Quality": 3, Workflow: 2, Volume: 1 };
  const unique = new Map<string, AlertItem>();
  for (const item of alerts) unique.set(item.id, item);
...
export function computeRootCauses(records: ServiceRecord[], quality: QualityReport): RootCauseItem[] {
  const trusted = trustedRecords(records, quality);
  const end = analysisDateFor(trusted);
  const start = addDays(end, -29);
  ...
  const dimensions: Array<{ label: RootCauseItem["dimension"]; value: (record: ServiceRecord) => string }> = [
    { label: "Category", value: (record) => record.category },
    { label: "Team", value: (record) => record.team },
    { label: "Location", value: (record) => record.location },
    { label: "Channel", value: (record) => record.channel },
  ];
  const output: RootCauseItem[] = [];
  for (const dimension of dimensions) {
    ...
    for (const name of names) {
      ...
      if (group.length < 5 || groupMisses.length === 0) continue;
...
export function buildRecommendations(alerts: AlertItem[], rootCauses: RootCauseItem[], quality: QualityReport): Recommendation[] {
  ...
  const priorityRank: Record<Severity, number> = { Critical: 4, High: 3, Medium: 2, Low: 1 };
  return recommendations.sort((a, b) => priorityRank[b.priority] - priorityRank[a.priority]).slice(0, 7);
}

function periodRecords(records: ServiceRecord[], category: string, start: Date, end: Date): ServiceRecord[] {
  return records.filter((record) => record.category === category && inWindow(record.closedAt, start, end));
}

export function evaluateInterventions(records: ServiceRecord[], quality: QualityReport): InterventionResult[] {
  const trusted = trustedRecords(records, quality);
...
function dailyFlowSeries(records: ServiceRecord[], start: Date, days: number): Array<{ date: Date; weekday: number; created: number; closed: number; net: number }> {
  const output: Array<{ date: Date; weekday: number; created: number; closed: number; net: number }> = [];
  for (let offset = 0; offset < days; offset += 1) {
...
function weekdayMeans(series: Array<{ weekday: number; created: number; closed: number }>): Array<{ created: number; closed: number }> {
...
function mixConstraint(records: ServiceRecord[], end: Date): number {
  ...
  if (open.length === 0 || closed.length === 0) return 0;
...
function agingConstraint(records: ServiceRecord[], end: Date): number {
  const open = records.filter((record) => validDate(record.createdAt) && record.createdAt <= end && (!validDate(record.closedAt) || record.closedAt > end));
...
function backtestScenarioModel(series: Array<{ date: Date; weekday: number; created: number; closed: number; net: number }>) {
  if (series.length < 42) {
...
export function runScenario(records: ServiceRecord[], quality: QualityReport, demandChangePct: number, capacityChangePct: number, weeks = 6): ScenarioResult {
  const trusted = trustedRecords(records, quality);
  const end = startOfDay(analysisDateFor(trusted));
  const recentStart = addDays(end, -27);
...
export function buildAnalytics(records: ServiceRecord[], quality: QualityReport): AnalyticsBundle {
...
export function groundedSummary(bundle: AnalyticsBundle, datasetName: string): string {
  ...
  if (topAlert) paragraphs.push(`The highest-priority signal is “${topAlert.title}.” ${topAlert.description}`);
  if (topRoot) paragraphs.push(`${topRoot.name} is the largest identified contributor to recent SLA misses at ${topRoot.contributionPct.toFixed(1)}% of misses and ${topRoot.relativeRisk.toFixed(2)}× relative risk.`);
  if (recommendations[0]) paragraphs.push(`Management priority: ${recommendations[0].action}`);
