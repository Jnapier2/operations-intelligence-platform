import type { DailyPoint, DimensionMetric, ForecastPoint } from "./types.js";
...
export function safeCssToken(value: unknown, fallback = "unknown"): string {
...
function points(values: number[], width: number, height: number, padding = 10): string {
...
export function sparkline(values: number[], direction: "good-up" | "good-down" | "neutral" = "neutral"): string {
  const width = 132;
  const height = 36;
...
export function lineChart(data: DailyPoint[]): string {
  const width = 880;
  const height = 270;
  ...
  const x = (index: number) => margin.left + (index / Math.max(data.length - 1, 1)) * chartWidth;
  const y = (value: number) => margin.top + chartHeight - (value / maximum) * chartHeight;
...
export function horizontalBars(data: DimensionMetric[], options: { suffix?: string; maximum?: number; inverse?: boolean } = {}): string {
...
export function gauge(score: number, label: string): string {
...
export function forecastChart(data: ForecastPoint[]): string {
  const width = 760;
  const height = 240;
