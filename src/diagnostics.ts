...
function readLog(): LogEvent[] {
  try {
    return JSON.parse(localStorage.getItem(LOG_KEY) ?? "[]") as LogEvent[];
  } catch {
    return [];
  }
}

export function logEvent(level: LogEvent["level"], message: string, details = ""): void {
...
async function sendCapsule(capsule: Record<string, unknown>): Promise<void> {
  try {
...
export function reportCritical(error: unknown, context: string): void {
