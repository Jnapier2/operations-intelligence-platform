...
function missedSla(record: ServiceRecord): boolean {
  return isClosed(record) && record.slaHours !== null && record.resolutionHours !== null && record.resolutionHours > record.slaHours;
}

function processPath(record: ServiceRecord): string[] {
  const path = ["Request created", `${record.channel || "Unknown"} intake`, record.team || "Unassigned team", record.category || "Uncategorized"];
  if (record.priority === "Critical" || record.priority === "High") path.push("Priority handling");
  if (record.reopened) path.push("Rework / reopened");
  if (missedSla(record)) path.push("SLA exception");
  path.push(isClosed(record) ? "Resolved" : "Open work");
  return path;
}

function buildProcessVariants(records: ServiceRecord[]): ProcessVariant[] {
  const groups = new Map<string, { path: string[]; records: ServiceRecord[] }>();
  ...
  return [...groups.entries()]
    .map(([signature, group], index) => {
      ...
      const resolution = closed.map((item) => item.resolutionHours).filter((value): value is number => value !== null && Number.isFinite(value));
      return {
...
function metricGroups(records: ServiceRecord[], kind: ProcessBottleneck["kind"], key: (record: ServiceRecord) => string): ProcessBottleneck[] {
...
function buildBottlenecks(records: ServiceRecord[]): ProcessBottleneck[] {
  const items = [
...
function rootFactorCandidates(record: ServiceRecord): Array<[string, string]> {
  ...
  if (record.reopened) result.push(["Workflow", "Reopened request"]);
  if (record.resolutionHours !== null && record.slaHours !== null && record.resolutionHours > record.slaHours * 1.5) result.push(["Cycle", "Resolution exceeded 150% of SLA"]);
  return result;
}

function buildRootFactors(records: ServiceRecord[]): ProcessRootFactor[] {
  ...
  const groups = new Map<string, { condition: string; records: ServiceRecord[] }>();
...
function buildObjectGraph(records: ServiceRecord[]): { nodes: ObjectGraphNode[]; edges: ObjectGraphEdge[] } {
  const nodeMap = new Map<string, { type: ObjectGraphNode["type"]; label: string; volume: number; miss: number }>();
  const edgeMap = new Map<string, ObjectGraphEdge>();
  const touchNode = (type: ObjectGraphNode["type"], label: string, miss: boolean): string => {
    const id = `${type.toLowerCase()}:${label || "Unassigned"}`;
    const current = nodeMap.get(id) ?? { type, label: label || "Unassigned", volume: 0, miss: 0 };
    current.volume += 1;
    if (miss) current.miss += 1;
    nodeMap.set(id, current);
    return id;
  };
  const touchEdge = (source: string, target: string, relationship: string): void => {
...
function buildPulse(analytics: AnalyticsBundle, bottlenecks: ProcessBottleneck[]): PulseInsight[] {
  const topAlert = analytics.alerts[0];
  const topBottleneck = bottlenecks[0];
  const items: PulseInsight[] = [
    {
      id: "pulse-sla",
...
function opportunityGroups(records: ServiceRecord[]): AutomationOpportunity[] {
  const groups = new Map<string, ServiceRecord[]>();
  for (const record of records) {
  ...
  return [...groups.entries()].map(([name, values], index) => {
    ...
    for (const value of values) subcategories.set(value.subcategory, (subcategories.get(value.subcategory) ?? 0) + 1);
    ...
    return {
      ...
      complexity: (score >= 80 ? "Low" : score >= 65 ? "Medium" : "High") as AutomationOpportunity["complexity"],
...
function buildAlertNoise(analytics: AnalyticsBundle): AlertNoiseSummary {
...
export function buildIntelligence(records: ServiceRecord[], quality: QualityReport, analytics: AnalyticsBundle): IntelligenceBundle {
...
export function classifyRequestText(text: string): ClassificationSuggestion {
...
export function answerOperationsQuestion(query: string, analytics: AnalyticsBundle, intelligence: IntelligenceBundle, datasetName: string): GroundedAnalystAnswer {
